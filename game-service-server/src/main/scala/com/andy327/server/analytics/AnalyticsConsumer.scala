...
import com.andy327.server.analytics.GameEventCodecs.decoder
...
/** Consumes analytics events off the `game-analytics` channel and folds each one into [[GameMetrics]].
  ...
  * tests), are decoded to `GameEvent`, and recorded. A message that fails to decode is logged and skipped so
