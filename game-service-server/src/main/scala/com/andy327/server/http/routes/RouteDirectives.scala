...
/** Common HTTP route directives for parsing and validating parameters such as UUIDs and GameTypes.
  ...
  * converted into typed domain values. Each method returns a `Directive1` that either provides a validated value or
  * rejects the request with a 400 Bad Request response.
  *
  * Example use:
  * {{{
