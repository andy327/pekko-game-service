...
/** An actor representing an individual player's session.
  ...
  * is the transport layer's responsibility (see `WebSocketRoutes`), so this actor
...
object PlayerActor {
  ...
  /** Protocol for the outbound stream backing a player session.
    *
    * The ActorSource in `WebSocketRoutes` is materialized over this type so that the
